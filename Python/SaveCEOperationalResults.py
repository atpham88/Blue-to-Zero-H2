...
def saveCapacExpOperationalData(ceModel,genFleet,newTechsCE,hoursForCE,transRegions,lineLimits,H2lineLimits,resultsDir,modelName,year):
    ...
    saveH2LineFlows(ceModel,H2lineLimits,hoursForCE,resultsDir,modelName,year)
...
def saveGeneratorResults(ceModel,gens,hoursForCE,resultsDir,modelName,year,newTechs=False):
    for v in ['vGen','vRegup','vFlex','vCont','vTurnon','vTurnoff','vOnoroff','vCharge','vStateofcharge','vELCharge','vH2TCharge']:
...
def saveH2LineFlows(ceModel,H2lineLimits,hoursForCE,resultsDir,modelName,year,varName = 'vH2Lineflow'):
    if varName in [i.name for i in ceModel.out_db]:
        df = pd.DataFrame(columns=H2lineLimits['GAMS Symbol'].values,index=hoursForCE.index)
        for rec in ceModel.out_db[varName]: df.loc[rec.key(1),rec.key(0)] = rec.level
        df.to_csv(os.path.join(resultsDir,varName + modelName + str(year) + '.csv'))
